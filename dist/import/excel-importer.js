...
const dateFromExcel = (value) => {
    if (typeof value === 'number')
        return new Date(`${excelSerialToDate(value)}T00:00:00.000Z`);
...
const rows = (workbook, sheetName, range = 0) => {
    const sheet = workbook.Sheets[sheetName];
    if (!sheet)
        return [];
    return XLSX.utils.sheet_to_json(sheet, { defval: '', range });
};
...
export function extractMonthlyCostValues(raw, rowIndex, type) {
...
export function buildMonthlyCostPayload(type, month, year, value) {
...
export async function importWorkbook(path, prisma) {
...
async function importInterns(prisma, workbook, sheetName, type) {
    ...
    for (let rowIndex = startIndex; rowIndex < raw.length; rowIndex += 1) {
        ...
        if (!name || !startDate || !endDate || !division)
            continue;
...
async function importMonthlyCosts(prisma, internId, type, monthlyCosts) {
...
async function importPlans(prisma, workbook) {
    const raw = matrix(workbook, 'Rencana Magang ');
    const headers = (raw[2] ?? []).map((value) => clean(value));
    const data = raw.slice(4).map((line) => Object.fromEntries(headers.map((header, index) => [header, line[index] ?? ''])));
    let imported = 0;
...
async function importReplacement(prisma, workbook) {
...
async function importEvaluations(prisma, workbook) {
...
//# sourceMappingURL=excel-importer.js.map