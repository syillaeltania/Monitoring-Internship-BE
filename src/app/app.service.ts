...
export class AppService {
    ...
    if (existingIntern) {
      ...
      if (plan) {
        ...
        const calculatedStatus = calculateStatus(
          ...
          new Date(),
          updated.manualStatus === 'TERMINATED'
        );
  ...
  async reorderTeamRequirements(orders: { id: string; orderNo: number }[]) {
    await this.prisma.$transaction(
      orders.map((item) =>
        this.prisma.teamRequirement.update({
          where: { id: item.id },
  ...
  async toggleTeamVisibility(id: string, isHidden: boolean) {
  ...
  private async _recalculateTeamRequirements() {
    ...
    const normalizeDivision = (div: string) => {
      ...
      if (d === 'NEW BUSINESS' || d === 'NB') return 'NB';
      return d;
    };

    const normalizeKey = (div: string, tm: string, pos?: string | null) => {
      const upperTeam = tm.trim().toUpperCase().replace(/\s+/g, ' ');
      const upperPosition = (pos || '').toUpperCase().replace(/\s+/g, ' ');
      let normalizedTeam = tm.trim();
      
      if (upperTeam === 'LOGISTIK' || upperTeam === 'LOG') normalizedTeam = 'LOGISTIC';
      else if (upperTeam === 'SMART' && upperPosition.includes('PR')) normalizedTeam = 'SMART (PR)';
      else if (upperTeam === 'SMART' && upperPosition.includes('ADMIN')) normalizedTeam = 'SMART (Admin)';
      else if (upperTeam === 'SMART' && upperPosition.includes('WEBDEV')) normalizedTeam = 'SMART (Webdev)';
      else if (upperTeam === 'AM' && upperPosition.includes('QA')) normalizedTeam = 'AM (QA)';
      else if (upperTeam === 'AM' && upperPosition.includes('TW')) normalizedTeam = 'AM (TW)';
      else if (upperTeam === 'NB-3 (PGN BILIING)' || upperTeam === 'NB-3 (PGN BILLING)') normalizedTeam = 'NB-3 (PGN Billing)';
      else if (upperTeam === 'NB-4 (PEGADAIAN)' || upperTeam === 'NB-4 (PEGADAIAN)') normalizedTeam = 'NB-4 (Pegadaian)';

      // MSOS & SQ mappings
      else if (upperTeam === 'MSOS-1' || upperTeam === 'MSOS 1' || upperTeam === 'MSO-1' || upperTeam === 'MSO 1') normalizedTeam = 'MSO 1';
      else if (upperTeam === 'MSOS-2' || upperTeam === 'MSOS 2' || upperTeam === 'MSO-2' || upperTeam === 'MSO 2') normalizedTeam = 'MSO 2';
      else if (upperTeam === 'MSOS-3' || upperTeam === 'MSOS 3' || upperTeam === 'MSO-3' || upperTeam === 'MSO 3') normalizedTeam = 'MSO 3';
      else if (upperTeam === 'MSOS-4' || upperTeam === 'MSOS 4' || upperTeam === 'MSO-4' || upperTeam === 'MSO 4') normalizedTeam = 'MSO 4';
      else if (upperTeam === 'MSOS-5' || upperTeam === 'MSOS 5' || upperTeam === 'MSO-5' || upperTeam === 'MSO 5') normalizedTeam = 'MSO 5';
      else if (upperTeam === 'MSOS-6' || upperTeam === 'MSOS 6' || upperTeam === 'MSO-6' || upperTeam === 'MSO 6') normalizedTeam = 'MSO 6';
      else if (upperTeam === 'SQ') normalizedTeam = 'Software Quality';
    ...
    const internGroups = new Map<string, typeof allInterns>();
